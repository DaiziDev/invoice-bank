...
import { PwaInstallButton, Screen, Body } from "../ui/shell";
import {
  Head,
  GroupedOps,
  KV,
...
function BalanceCard({ a, title }: { a: Account; title: string }) {
  ...
  return (
    <div
      className="relative shrink-0 w-full snap-center rounded-[26px] px-5 pt-5 pb-[17px] text-white overflow-hidden"
      style={{
        background:
    ...
    >
      ...
      {pend > 0 && (
        <span
          className="absolute top-4 right-[18px] mono text-[9px] tracking-[.12em] uppercase px-2 py-1 rounded-full
      ...
      <div className="relative mb-3.5">
        ...
        <div className="mono text-[10.5px] opacity-55 tracking-[.04em]">
          {a.number}
        </div>
      </div>
      <span className="relative amt block text-[33px] leading-none">
        {fmt(a.balanceConfirmed)}
      ...
      <div className="relative mt-[11px] pt-[11px] border-t border-white/15 flex justify-between items-baseline text-xs">
        ...
        <span className={`amt ${pend ? "text-[#F0B58C]" : ""}`}>
          {money(available(a))}
        </span>
      </div>
    </div>
...
export function Home() {
  ...
  const cards = walletMode
    ? ([
        wallet && { a: wallet, title: t("myWallet") },
        bankAcc && { a: bankAcc, title: t("myBank") },
      ].filter(Boolean) as { a: Account; title: string }[])
  ...
  return (
    <Screen>
      <Head
        noBack
        title={bank.bankName}
        sub={`${t("hello")}, ${s.profile.first}`}
        right={
          <div className="flex items-center gap-1">
            <PwaInstallButton light />
            <button
              onClick={() => nav("/notifications")}
              aria-label={t("notifications")}
              className="relative w-9 h-9 rounded-[11px] grid place-items-center bg-surface-2"
            >
              <Ic.Bell />
              {unread > 0 && (
                <span className="absolute top-[7px] right-[7px] w-[7px] h-[7px] rounded-full bg-queued" />
              )}
            </button>
          </div>
        }
      ...
      <Body className="stagger">
        <div>
          <div
            className="flex gap-3 overflow-x-auto snap-x snap-mandatory no-sb -mx-[18px] px-[18px]"
            onScroll={(e) =>
              setPage(
                Math.round(
          ...
          >
            {cards.map((c) => (
              <BalanceCard key={c.a.id} a={c.a} title={c.title} />
            ))}
          </div>
          {cards.length > 1 && (
            <div className="flex gap-1.5 justify-center mt-[11px]">
              {cards.map((_, i) => (
                <i
                  key={i}
                  className={`h-[5px] rounded-full transition-all ${i === page ? "w-[17px] bg-brand" : "w-[5px] bg-[var(--line-strong)]"}`}
                />
              ))}
            </div>
          )}
          <div className="mono text-[9.5px] text-muted text-center mt-2">
            {t("upToDate")} {hm(new Date().toISOString())}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between gap-3 card px-3.5 py-3">
            <div>
              ...
              <div className="text-[11.5px] text-muted mt-1.5">
                {t("tierLimitL")} : <b>{money(effectiveLimits(s).single)}</b>
              </div>
            </div>
            {s.tier < 3 && (
              <button
        ...
        <div className="grid grid-cols-4 gap-2">
          {quick.map((q) => (
            ...
            >
              ...
              <span className="text-[10.5px] text-center leading-tight text-muted font-medium">
                {q.lb}
              </span>
            </button>
          ))}
        ...
        {walletMode && !bankAcc && (
          <button
            className="opt card border-dashed border-[var(--line-strong)]"
            onClick={() => nav("/link")}
          >
            <span className="w-10 h-10 rounded-xl bg-brand-soft text-brand grid place-items-center shrink-0">
              <Ic.Bank />
            </span>
        ...
        {recent.length ? (
          <div className="card">
            {recent.map((x) => (
              <OpRow key={x.id} x={x} onClick={() => nav(`/app/txn/${x.id}`)} />
            ))}
          </div>
        ) : (
          <div className="card px-4 py-6 text-center text-[12.5px] text-muted">
            {t("noActivity")}
          </div>
        )}
        <div className="mono text-[9.5px] text-muted text-center">
          {t("demoData")}
        </div>
      </Body>
    </Screen>
...
export function History() {
  ...
  const list = useMemo(() => {
    ...
    if (dir !== "all")
      l = l.filter((x) => x.direction === (dir === "in" ? "CREDIT" : "DEBIT"));
    if (q) {
      const n = q.toLowerCase();
      l = l.filter(
        (x) =>
          x.label.toLowerCase().includes(n) ||
          x.reference.toLowerCase().includes(n),
      );
    }
    return l;
  }, [s, q, dir]);
      ...
      <Body>
          ...
          <div className="flex items-center gap-3 rounded-[18px] px-4 py-4 bg-[rgba(169,50,38,.09)] text-danger">
            ...
            <div>
              ...
              <div className="text-[11.5px] opacity-80 mt-0.5">
                {t(x.failureReason as "overSingle")} — {t("limitIs")}{" "}
                {money(effectiveLimits(s).single)}
              </div>
            </div>
          </div>
        )}
        {x.status === "QUEUED" && (
          <div className="sim-note">{t("queuedD")}</div>
        )}

        <div className="card px-4 py-2">
          <KV k={t("reference")} v={x.reference} mono />
          <KV
            k={credit ? t("sender") : t("beneficiary")}
            v={x.counterparty.name}
          />
          {x.counterparty.identifier && (
            <KV k={t("number")} v={x.counterparty.identifier} mono />
          )}
        ...
        {x.status === "SETTLED" && (
          <button
            ...
            onClick={() => nav(`/receipt/${x.id}`)}
          >
            <Ic.Receipt s={18} /> {t("viewReceipt")}
          </button>
        )}
      </Body>
    </Screen>
  ...
  return (
    <Screen>
      <Head
        ...
        right={
          <button
            className="w-9 h-9 rounded-[11px] grid place-items-center bg-surface-2"
            onClick={() =>
              toast(
                s.lang === "fr"
                  ? "Ajout de bénéficiaire — périmètre v1"
                  : "Add payee — v1 scope",
              )
            }
          >
          ...
          <div className="card m-[18px]">
            {list.map((b) => (
              <button
                ...
                className="opt"
                onClick={() =>
                  nav(`/transfer/form/${b.kind}`, { state: { benef: b } })
                }
              >
                <span
                  className="w-10 h-10 rounded-full grid place-items-center display text-sm font-semibold text-white shrink-0"
                  style={{ background: `hsl(${hue(b.name)} 42% 38%)` }}
                >
                  {b.name[0]}
                </span>
        ...
        {s.notifs.length ? (
          <div className="card m-[18px]">
            {s.notifs.map((n) => (
              <div
                key={n.id}
                className="relative flex items-center gap-3 px-4 py-3 bg-surface border-b border-[var(--line)] last:border-0"
              >
                {n.kind === "queued" && (
    ...
    <Screen>
      ...
      <Body className="!p-0 gap-0">
        <div className="card m-[18px]">
          <div className="opt">
            ...
            <div className="flex gap-2">
              {(["fr", "en"] as const).map((l) => (
                <button
                  key={l}
                  className={`chip ${s.lang === l ? "chip-on" : ""}`}
                  onClick={() => setState({ lang: l })}
                >
                  {l.toUpperCase()}
                </button>
              ))}
          ...
          <button
            className="opt"
            onClick={() => setState({ dataSaver: !s.dataSaver })}
          >
            <span className="w-10 h-10 rounded-xl bg-brand-soft text-brand grid place-items-center shrink-0">
              <Ic.WifiOff />
            </span>
